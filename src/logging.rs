use log::LevelFilter;
use log4rs::{self, append::console::ConsoleAppender, config::{self, Appender, Logger}, encode::pattern::PatternEncoder, Config};
...
pub fn init() -> Result<log4rs::Handle> {
    ...
    let appender = Appender::builder()
        .build(APPENDER_NAME, Box::new(stdout_appender));
